...
**Session:** 4
**Last updated:** 2026-09-25 — Session 4, v1.1 built and Half B live-testing in progress
...
- [ ] (v1.1 revision) GATE, half B (the named people) — in progress: builder invited both placeholders and assigned Revisore Pilota's role; Utente Pilota still has no role assigned. Still needed: log in as each of the three and confirm every `no` is refused and every `own` returns only the right rows on the real screens
- [x] (v1.1 revision) Push to main → Netlify auto-deploys
- [x] (v1.1 revision) Invite Revisore Pilota (lucchini.v+revisore@gmail.com) and Utente Pilota (lucchini.v+utente@gmail.com) via Authentication → Users → Invite — done by builder 2026-09-25
- [ ] (v1.1 revision) Assign Utente Pilota's role to `user` in Manage Users (Revisore Pilota's `reviewer` role is already assigned)
- [ ] (v1.1 revision) Log in as Revisore Pilota and Utente Pilota (their Supabase-issued credentials) and confirm on the real screens: Review Status is editable for Revisore Pilota, read-only text for Utente Pilota; neither sees or can reach Manage Users
...
## Build decisions
- Manage Users' role dropdown stays Administrator/Reviewer/User (technical values, unchanged). Added a separate, non-editable "Position" column showing the real-world job title behind each role (Head of Sustainability / HSE Manager / Procurement Manager), derived client-side from `u.role` via a `ROLE_POSITIONS` map — not stored in the database, since it's a 1:1 label for the existing role, not new data. Row order: Position → Email → Role dropdown (builder's request).
...
- Found live during Half B, 2026-09-25: the builder changed her own account from `administrator` to `user` while assigning a placeholder's role in Manage Users (self-edit is allowed by design, per the lockout-prevention decision above) — she lost her own access to Manage Users on the next refresh. Restored her role directly in the database. Added a `confirm()` prompt in Manage Users' role selector: it fires only when the signed-in Administrator is about to change their *own* row away from `administrator`, warning that they will lose Manage Users access; cancelling reverts the dropdown to the real saved value. This does not change any RLS rule — an Administrator can still choose to demote themselves — it only adds a warning against doing it by accident.
